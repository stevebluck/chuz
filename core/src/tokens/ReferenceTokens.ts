...
import { HashMap, Array, Timestamp, makeUuid, Duration } from "@chuz/prelude";
...
export class ReferenceTokens<A> implements Tokens<A> {
  ...
  issue = (value: A, timeToLive: Token.TimeToLive): Effect.Effect<Token<A>> => {
    return Effect.gen(this, function* () {
      ...
      const expiresAt = new Timestamp({ value: addMilliseconds(time, Duration.toMillis(timeToLive.duration)) });
  ...
  lookup = (token: Token<A>): Effect.Effect<A, Token.NoSuchToken> => {
    return this.clock.currentTimeMillis.pipe(
      ...
      Effect.flatMap((time) => Ref.get(this.state).pipe(Effect.flatMap((s) => s.lookup(token, new Timestamp({ value: time }))))),
      Effect.catchTag("NoSuchElementException", () => new Token.NoSuchToken()),
    );
  };

  findByValue = (value: A): Effect.Effect<Array<Token<A>>> => Ref.get(this.state).pipe(Effect.map((s) => s.findByValue(value)));
...
class State<A> {
  constructor(
    private readonly table: HashMap.HashMap<Token<A>, [A, Timestamp]>,
  ...
  revokeMany = (token: Array<Token<A>>): State<A> => new State(HashMap.removeMany(this.table, token), this.eq);

  findByValue = (a: A): Array<Token<A>> => HashMap.filter(this.table, ([value]) => this.eq(value, a)).pipe(HashMap.keys, Array.fromIterable);
}
