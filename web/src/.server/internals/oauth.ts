...
export class State extends S.Class<State>("State")({
  ...
  static login = (provider: Provider) => makeUuid.pipe(Effect.map((value) => this.make({ provider, intent: Intent.Login, value: State.fields.value.make(value) })));

  static register = (provider: Provider) => makeUuid.pipe(Effect.map((value) => this.make({ provider, intent: Intent.Register, value: State.fields.value.make(value) })));

  static toString = (state: State) => toString(state).pipe(Either.mapLeft(() => InvalidState({ error: "auth state could not be converted to string" })));
...
export const StateFromString = S.transformOrFail(S.String, State, {
  ...
  decode: (str) => {
    const arr = str.split("+");
    return S.decodeUnknown(State)({ intent: arr[0], value: arr[1], provider: arr[2] }).pipe(Effect.mapError((e) => new PR.Type(S.String.ast, str, "invalid auth state string")));
  },
});
