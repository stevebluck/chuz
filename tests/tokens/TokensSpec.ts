...
import { NoSuchToken, TimeToLive, Token, Tokens } from "~/core/tokens/Tokens";
  ...
  export const run = (c: Effect.Effect<Tokens<number>>) => {
    asyncProperty("tokens associate with the values provided at the time of issue", fc.integer(), (value) =>
      Effect.gen(function* (_) {
        ...
        const token = yield* _(tokens.issue(value, TimeToLive(Duration.toMillis("1 days"))));
    ...
    asyncProperty("lookup fails when tokens have expired according to their TTL", fc.integer(), (value) =>
      Effect.gen(function* (_) {
        const tokens = yield* _(c);
        const token = yield* _(tokens.issue(value, TimeToLive(1)));
    ...
    asyncProperty("lookup fails when the token does not exist", fc.integer(), (value) =>
      Effect.gen(function* (_) {
        const tokens = yield* _(c);
        const token = Token<number>(value.toString());
        const error = yield* _(tokens.lookup(token).pipe(Effect.flip));
        expect(error).toEqual(new NoSuchToken());
      }).pipe(Effect.runPromise),
    );
    ...
    asyncProperty("tokens may be revoked in bulk", fc.array(fc.integer(), { minLength: 0, maxLength: 10 }), (values) =>
      Effect.gen(function* (_) {
        const tokens = yield* _(c);
        const ts = yield* _(
          Effect.forEach(values, (value) => tokens.issue(value, TimeToLive(Duration.toMillis("1 days")))),
        );
    ...
    asyncProperty("tokens may be found and revoked in bulk by value", fc.integer(), (value) =>
      Effect.gen(function* (_) {
        const tokens = yield* _(c);
        const token0 = yield* _(tokens.issue(value, TimeToLive(Duration.toMillis("1 days"))));
        const token1 = yield* _(tokens.issue(value, TimeToLive(Duration.toMillis("1 days"))));
        const token2 = yield* _(tokens.issue(value, TimeToLive(Duration.toMillis("1 days"))));
