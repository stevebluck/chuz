...
import { SpecConfig, defaultSpecConfig } from "tests/SpecConfig";
import { TestBench } from "tests/TestBench";
import { afterAll, describe, expect } from "vitest";
import { Credentials, Email, NoSuchToken, Password, User, Users } from "~/core";
  ...
  export const run = (testBench: Effect.Effect<TestBench>, config: SpecConfig = defaultSpecConfig) => {
      ...
      (register) =>
        Effect.gen(function* (_) {
          ...
          expect(session.user.value).toEqual(
            User.make(register.firstName, register.lastName, register.credentials.email, register.optInMarketing),
          );
    ...
    asyncProperty(
      ...
      (register) =>
        Effect.gen(function* (_) {
          ...
          expect(noSuchTokenError).toEqual(new NoSuchToken());
        }).pipe(Effect.runPromise),
      config,
    );
    ...
    describe("Update", () => {
      asyncProperty(
        ...
        fc.tuple(Arbs.Users.Register, Arbs.Users.DraftUser),
        ([register, draft]) =>
          Effect.gen(function* (_) {
            const { users, session } = yield* _(seededBench(register));
      ...
      asyncProperty(
        ...
        ([register, newPassword]) =>
          Effect.gen(function* (_) {
            ...
            expect(error0).toEqual(new NoSuchToken());
            expect(error1).toEqual(new NoSuchToken());
          }).pipe(Effect.runPromise),
        config,
      );
    ...
    const seededBench = (register: Arbs.Users.Register) =>
      Effect.gen(function* (_) {
        const { users, hasher } = yield* _(testBench);
        const session = yield* _(registerUser(users)(register));

        return { session, users, hasher };
      });
